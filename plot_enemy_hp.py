...
def init():
    global last_room, last_hp, samus_hp
    last_room = 0
    last_hp = 0
    samus_hp = 0
...
def animate(i, xs, ys):
    ...
    if change:
        ...
        #xs.append(i) #datetime.datetime.now().strftime('%H:%M:%S.%f'))
        if current_room not in boss_fights:
            boss_fights[current_room] = {"xs" : [], "ys" : [], "average" : []}
        boss_fights[current_room]["xs"].append(i)
        boss_fights[current_room]["ys"].append(current_hp)
        boss_fights[current_room]["average"].append(sum(boss_fights[current_room]["ys"])/len(boss_fights[current_room]["ys"]))
        ...
        # Draw x and y lists
        ax.clear()
        ax.plot(boss_fights[current_room]["xs"][-20:], boss_fights[current_room]["average"][-20:])
        ax.xaxis.set_major_formatter(plt.NullFormatter())
        # Format plot
        #plt.xticks(rotation=45, ha='right')
       # plt.subplots_adjust(bottom=0.30)
        #plt.title('Fight over time')
        plt.tight_layout()
        #plt.ylabel('Enemy HP')


# Set up plot to call animate() function periodically
ani = animation.FuncAnimation(fig, animate, init_func=init, fargs=(xs, ys), interval=1000)
plt.show()
